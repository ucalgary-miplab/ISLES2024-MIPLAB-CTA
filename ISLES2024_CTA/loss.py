...
class DiceLoss(_Loss):
    def __init__(
        ...
        if classes is not None:
            ...
            classes = self.to_tensor(classes, dtype=torch.long)
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == BINARY_MODE:
            y_true = self._hard_max(y_true, axis=1)
            y_pred = self._hard_max(y_pred, axis=1)

            # Return probabilities for the non-healthy class only
            y_true = torch.split(y_true, 1, 1)[1]
            y_pred = torch.split(y_pred, 1, 1)[1]
    ...
    def _hard_max(self, tens, axis):
        ...
        eps_hot = torch.clamp(tens - tensmax + torch.finfo(tens.dtype).eps, min=0)
        one_hot = eps_hot / torch.finfo(tens.dtype).eps
        return one_hot

    def to_tensor(self, x, dtype=None) -> torch.Tensor:
    ...
    def soft_dice_score(
        ...
        if dims is not None:
            ...
            cardinality = torch.sum(output**2, dim=dims) + torch.sum(target**2, dim=dims)
        else:
            intersection = torch.sum(output * target)
            cardinality = torch.sum(output**2) + torch.sum(target**2)
